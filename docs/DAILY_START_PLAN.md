...
## 0.5 每日偏离检查流程(2026-06-21 新增)

**每次开始新工作前,必须执行此检查流程,防止偏离项目目标。**

### 步骤1:读取核心目标
```text
项目目标:基于量化因子的A股短线择股择时系统
核心任务:让买卖点基于资金流、相对换手、情绪因子等新量化因子
禁止:单纯技术指标凑数触发
```

### 步骤2:检查当前工作是否偏离
问自己:
1. 当前工作是否直接服务于"基于量化因子"的核心目标?
2. 当前工作是在重构买点逻辑,还是在完善周边功能?
3. 如果用户说"买点还是旧的",我是否还在做技术完善而非逻辑重构?

### 步骤3:发现偏离立即纠正
如果发现偏离:
1. 立即停止当前技术完善工作
2. 回到项目目标:重构买点逻辑
3. 优先完成:定义新条件集 → 更新策略配置 → 实现判断逻辑

### 步骤4:记录偏离与纠正
在 `docs/SESSION_HANDOFF.md` 中记录:
- 发现的偏离
- 采取的纠正措施
- 防止再次偏离的改进

### 偏离检查示例(2026-06-21 教训)
**问题**:用户说"买点还是旧的共振",但agent继续完善`skipped_signals`、A/B开关等周边功能。
**纠正**:应立即停止周边功能完善,开始买点逻辑重构。
**预防**:建立此检查流程,确保后续agent优先处理核心任务。
...
### 2.1 核心原则（每轮Agent必须遵守）

1. **不局限于用户提到的因子和参数**：允许主动引入新的量化因子、市场环境、资金流、情绪和相对换手判断，以提高买卖点准确性。

2. **禁止未来函数**：所有信号必须在T日收盘后生成，最早T+1开盘成交。禁止使用任何未来数据。

3. **禁止围绕单个 run 拟合**：不能针对单个回测结果临时补规则。所有规则修改必须有专业依据。

4. **禁止回测美化**：回测必须内置佣金、印花税、过户费、滑点四项成本，不能用收盘价模拟市价单。

5. **技术指标仅作为确认项**：MACD/KDJ/RSI/BOLL/MA等基础技术指标只能作为买卖点确认项，不能长期作为主驱动。

6. **发现偏离立即纠偏**：若执行中偏离"基于量化因子的短线择股择时"目标，必须在本计划中及时记录和纠正。

7. **每日回顾与计划**：每天回顾前面工作后，制定新的计划并更新本文档。所有Agent共同维护此文档。

### 2.2 量化因子优先级

**优先使用的量化因子**：
1. **资金流因子**：主力净流入、超大单净流入、资金流排名
2. **相对换手因子**：相对换手率、换手率分位、成交额分位
3. **市场环境因子**：大盘评分、北向资金、融资融券
4. **情绪因子**：涨跌停家数、封板率、市场情绪
...
1. **全盘理解业务系统**：不能只做表面修复或只改名称。
2. **基于证据的根因分析**：不能只靠联想和主观想象。
3. **先自测后通知**：完成代码修改后都要先自测，确认无问题后再通知用户。
4. **文档化交接**：关键规则、上下文、最新方案必须记录到固定交接文档，避免依赖聊天记录。

2026-06-21 纠偏记录：技术指标只能作为买卖点确认项，不能长期作为主驱动。允许引入新的量化因子、市场环境、资金流、情绪和相对换手判断，以提高买卖点准确性；但禁止未来函数、禁止围绕单个 run 拟合、禁止回测美化。
